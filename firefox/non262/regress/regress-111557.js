...
top.titles[i] = "Revision History";
i++; // Morten's JavaScript Tree Menu
...
function MTMDisplayItem(item, last) {
  ...
  if (MTMNN3 && !MTMLinkedSS) {
    ...
    if (item.submenu && item.url == "" && item.number == MTMClickedItem) {
      ...
      if (MTMTrackedItem && MTMTrackedItem == item.number) {
...
function MTMFetchIcon(testString) {
  ...
  for (i = 0; i < MTMIconList.items.length; i++) {
    if (MTMIconList.items[i].type == 'any' && testString.indexOf(MTMIconList.items[i].match) != -1) {
      ...
      if (MTMIconList.items[i].type == 'pre' && testString.indexOf(MTMIconList.items[i].match) == 0) {
        return MTMIconList.items[i].file;
      } else {
        if (MTMIconList.items[i].type == 'post' && testString.indexOf(MTMIconList.items[i].match) != -1) {
...
var MTMTableWidth = "100%"; // Name of the frame where the menu is to appear.

var MTMenuFrame = "tocmain"; // variable for determining whether a sub-menu always gets a plus-sign
// regardless of whether it holds another sub-menu or not

var MTMSubsGetPlus = true; // variable that defines whether the menu emulates the behaviour of
// Windows Explorer

var MTMEmulateWE = true; // Directory of menu images/icons

var MTMenuImageDirectory = "/ndk/doc/docui2k/menu-images/"; // Variables for controlling colors in the menu document.
// Regular BODY atttributes as in HTML documents.

var MTMBGColor = "#cc0000";
var MTMBackground = "";
var MTMTextColor = "white"; // color for all menu items

var MTMLinkColor = "#ffffcc"; // Hover color, when the mouse is over a menu link

var MTMAhoverColor = "#FF9933"; // Foreground color for the tracking & clicked submenu item

var MTMTrackColor = "#FF9933";
var MTMSubExpandColor = "#ffffcc";
var MTMSubClosedColor = "#ffffcc"; // All options regarding the root text and it's icon
...
var MTMRootFontSize = "-1"; // Font for menu items.

var MTMenuFont = "Verdana";
var MTMenuCSSize = "74%";
var MTMenuFontSize = "-1"; // Variables for style sheet usage
// 'true' means use a linked style sheet.

var MTMLinkedSS = false;
var MTMSSHREF = "style/menu.css"; // Whether you want an open sub-menu to close automagically
// when another sub-menu is opened.  'true' means auto-close

var MTMSubsAutoClose = false; // This variable controls how long it will take for the menu
...
MTMIconList = new IconList(); // examples:
...
var levelIndex = 0;
top.printList = "";
top.printCount = 0; // asign the menu handle to the created tree
...
function setLevels() {
  ...
  levelParents[currentLevel + 1] = index; // levelIndex is the child index under a branch
...
function buildTree() {
  ...
  for (var j4 = 0; j4 < navigationTree.length; j4++) {
    if (navigationTree[j4]["hasChild"]) {
      ...
      var lindex = navigationTree[j4]["levelIndex"]; //  document.write('treeNodes[' + pindex +'].items['+ lindex +'].MTMakeSubmenu(treeNodes['+(j4+1)+']);<br>');

      treeNodes[pindex].items[lindex].MTMakeSubmenu(treeNodes[j4 + 1]);
    }
  }

  menu = treeNodes[0]; //expand the second item to display the sub contents on first display
